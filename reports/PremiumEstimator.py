...
def mock_option_chain_test(ticker_symbol):
    ...
    try:
        ...
        st.warning(
            f"⚠️ Live data unavailable or empty (Yahoo delay). Generating **Mock Data** for testing. Reason: {e}"
        )
...
# 🔄 STATE MANAGEMENT
# =====================================================
if "row_deltas" not in st.session_state:
    st.session_state.row_deltas = {}
if "scan_results" not in st.session_state:
    st.session_state.scan_results = None
if "active_tab" not in st.session_state:
    st.session_state.active_tab = "💼 Trade Setup"
...
# ============================================================
    ...
    st.markdown(
        "Verify your option chain loading logic during off-hours or Yahoo delay periods."
    )
    ...
    with t_col1:
        test_ticker = (
            st.text_input("Enter Stock Ticker:", value="AAPL").upper().strip()
        )
    with t_col2:
    ...
    if test_btn and test_ticker:
        ...
        try:
            ...
            spot_price = ticker.history(period="1d")["Close"].iloc[-1]
            st.metric(
                label=f"Current Spot Price ({test_ticker})",
            ...
            # 3. Join Calls and Puts together on Strike
            ...
            chain_combined["distance"] = (
                chain_combined["strike"] - spot_price
            ).abs()
            ...
            atm_trade_time = max(
                final_chain.loc[closest_idx, "lastTradeDate_Call"],
                final_chain.loc[closest_idx, "lastTradeDate_Put"],
            )

            if isinstance(atm_trade_time, str):
                cleaned_time = atm_trade_time.split("+")[0]
                atm_datetime = datetime.strptime(
                    cleaned_time, "%Y-%m-%d %H:%M:%S"
                )
            else:
                atm_datetime = pd.to_datetime(atm_trade_time).replace(
                    tzinfo=None
                )

            # Missing pricing entirely
            if (
                final_chain.loc[closest_idx, "bid_Call"] == 0
                and final_chain.loc[closest_idx, "ask_Call"] == 0
            ):
                st.error(
                    f"❌ **Feed Status: Frozen.** No active Bid/Ask pricing detected yet. Yahoo's opening feed is inactive."
                )

            # If Bid & Ask are populated, it's a pass!
            elif (
                final_chain.loc[closest_idx, "bid_Call"] > 0
                and final_chain.loc[closest_idx, "ask_Call"] > 0
            ):
                st.success(
                    f"✅ **Validation Pass.** Live option chain pricing has successfully loaded!"
                )
            ...
            # Drop tracking columns before rendering
            render_chain = final_chain.drop(
                columns=[
                    "distance",
                    "lastTradeDate_Call",
                    "lastTradeDate_Put",
            ...
            # 6. Row Highlighting Function
            def highlight_atm(row):
                distance = abs(row["strike"] - spot_price)

                if row["bid_Call"] == 0 and row["ask_Call"] == 0:
                    return ["background-color: #fecaca; color: #991b1b;"] * len(
                        row
                    )

                if (
                    distance
                    == render_chain["strike"].sub(spot_price).abs().min()
                ):
                    return [
                        "background-color: #cbd5e1; font-weight: bold"
                    ] * len(row)
            ...
            styled_df = render_chain.style.apply(highlight_atm, axis=1).format(
                {
                    ...
                    "bid_Put": "${:.2f}",
                    "ask_Put": "${:.2f}",
                    "impliedVolatility_Call": "{:.2%}",
                    "impliedVolatility_Put": "{:.2%}",
                }
            )
