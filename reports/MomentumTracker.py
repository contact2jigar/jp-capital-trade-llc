...
def analyze_stock_full(symbol, target_expiry):
    # Forced sidebar header for visibility
    st.sidebar.header(f"🛠️ Debugging: {symbol}")
    st.sidebar.info(f"Step 1: Initializing Ticker for {symbol}")
    
    try:
        tk = yf.Ticker(symbol)
        
        # 1. History
        st.sidebar.info(f"Step 2: Fetching 2y History for {symbol}")
        hist = tk.history(period="2y") 
        if hist.empty or len(hist) < 10: 
            st.sidebar.error(f"❌ {symbol}: No price history found.")
            return None
        ...
        try:
            ...
            st.sidebar.warning(f"⚠️ Info fetch failed for {symbol}: {e}")
            info = {}

        # 3. Technicals (Standard logic)
        ...
        def format_ma(p, ma, color):
            if ma == 0: return "N/A"
            return f"<span style='color:{color}; font-weight:bold;'>${ma:.1f}</span>" if abs(p - ma) / ma <= 0.015 else f"${ma:.1f}"
        ...
        try:
            ...
            st.sidebar.warning(f"⚠️ Financials failed for {symbol}: {e}")
        ...
        # If it crashes anywhere, this red box WILL stay in the sidebar
