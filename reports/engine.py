...
def get_option_by_delta(ticker, expiry, side, target_delta):
    ...
    Side should be 'put' or 'call'. 
    Target delta is expected as a positive float (e.g., 0.15).
    """
    try:
        t = yf.Ticker(ticker)
...
def calculate_projections(
    l_m, i_m,
    llc_curr, ira_curr, k401_curr,
    k401_target_val,
    l_init, i_init, k_init,
    current_exp,
    ...
    total_aor = ((final_predictor - total_initial_basis) / total_initial_basis * 100) if total_initial_basis > 0 else 0
